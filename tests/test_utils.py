...
class TestGetFileNames(unittest.TestCase):
    ...
    def test_returns_files(self):
        ...
        self.assertEqual(getFileNames("./tests/images/"), file_names)

class TestGetMetaData(unittest.TestCase):
    
    def test_returns_metadata(self):
        file_names = getFileNames("./tests/images/")
        for j in range(len(file_names)):
            full_string = "./tests/images/" + file_names[j]
            print(getMetadata(full_string))

    def test_no_metadata(self):
        self.assertEqual(getMetadata("./tests/images/20220719_120940_0000.jpeg"), ['no EXIF'])
